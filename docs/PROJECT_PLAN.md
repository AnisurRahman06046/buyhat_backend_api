...
| 2 | Catalog | ◼ done — categories(tree)/brands/attributes(+options)/products/variants; dynamic **attribute inheritance** (category+ancestors) + **auto variant generation** (cartesian + `attribute_signature` idempotency) + publishing (DRAFT/ACTIVE/ARCHIVED). **Media done** — `@Global StorageModule` with swappable `STORAGE_PROVIDER` port (local-disk default ↔ S3, by `STORAGE_DRIVER`), `product_media` + `/products/:id/media` endpoints, first-image auto-primary, D11 publish-blocks-without-image, D10 max-variants env-configurable. Migrations applied; build+lint+**21 unit tests** + app-context DI smoke green. Design: `docs/PHASE_2_CATALOG.md`. |
| 3 | Inventory | ◼ done — per-variant `stock_item`, append-only `stock_movement` ledger, reservation system with **atomic conditional-UPDATE reserve** (no oversell, #1), delayed-job + sweeper expiry, `variant.created` consumer provisioning stock (catalog gets its own outbox + `catalog-events` queue). ADMIN+INVENTORY_MANAGER writes; 15-min TTL (env). Migrations applied; build+lint+**24 unit tests** green; **app-context contention smoke passed** (5-way concurrent reserve → exactly 1 ok/4 rejected; release/confirm/expire/ledger verified). Design: `docs/PHASE_3_INVENTORY.md`. |
| 4 | Cart | ◼ done — guest (`X-Guest-Id`) + customer carts, **merge-on-login (sums overlaps)**, price snapshots refreshed on read with per-line `priceChanged`, availability annotation via inventory, abandoned-cart sweeper. `CART_MAX_QTY_PER_LINE`=99. Migration applied; build+lint+**27 unit tests** green; **live smoke passed** (guest add → merge to user sums qty; MERGED guest cart emptied). Design: `docs/PHASE_4_CART.md`. |
...
| 7 | Promotions | ◼ done — `PromotionsService` facade (cart/orders call it, never touch tables): coupons (%/fixed/free-ship, min-purchase, category-restricted, global/per-user/**per-IP** caps #6), redeem-at-payment-success (idempotent unique (coupon,order)), flash sales (sweeper SCHEDULED→ACTIVE→ENDED, price locked at checkout #8), campaign CRUD. Checkout applies discount→grand_total; cart shows flash/coupon preview. ADMIN+MARKETING_MANAGER writes. Migration applied; build+lint+**58 unit tests** + e2e app-boot green. Design: `docs/PHASE_7_PROMOTIONS.md`. *(Also fixed a latent Phase 7 cart→promotions DI wiring gap caught later in Phase 8.)* |
| 8 | CMS | ◼ done — server-hydrated `GET /cms/homepage` (ordered sections enriched from same-module banners + catalog/promotions/reporting read-only seams), reorderable sections, scheduled window-active banners/hero-slider, audience-targeted popups (auth-state filtered), landing pages, CMS image upload via StorageModule. ADMIN+MARKETING_MANAGER writes; freeform-jsonb configs (hydration skips missing ids). Migration applied; build+lint+**68 unit tests** + **genuine e2e app-boot (real jest exit 0)** green. Design: `docs/PHASE_8_CMS.md`. |
| 9 | Reviews | ◼ done — any authed user reviews (auto `is_verified_purchase` via reviews→orders seam), **one review per (user,product)** (partial-unique→409), moderation (PENDING default, public=APPROVED, staff approve/reject=ADMIN+CUSTOMER_SUPPORT), rating aggregate recomputed + pushed to catalog via direct seam on any APPROVED-set change, owner edit (→PENDING) / owner-or-staff soft-delete, helpful counter. Migration applied; build+lint+**78 unit tests** + real e2e app-boot green. Design: `docs/PHASE_9_REVIEWS.md`. |
| 10 | Notifications | ◼ done — full multi-channel pipeline replacing the Phase 1 stub: `NotificationService.dispatch` seam → template (DB override of in-code defaults, `{{var}}`) → marketing preferences (transactional ignores) → BullMQ (3 attempts, backoff) → provider adapter → delivery-log row. Events: verify/reset, order paid/shipped/delivered, cart abandoned, low-stock, manual send. Rewired auth/inventory/orders/cart call sites; callers resolve `to` via `UsersService.getContactInfo` (no DI cycle). Port split (shared adapter `NotificationProviderModule` + feature `NotificationsModule`). Migration applied; build+lint+**86 unit tests** + real e2e app-boot green. Design: `docs/PHASE_10_NOTIFICATIONS.md`. |
| 11 | Reporting & Audit | ◼ done — denormalized fact tables (`order_fact`/`product_sales`/`customer_fact`) fed by **one-way service seams** at commit (idempotent ON CONFLICT counting; refunds set net_revenue), reporting never reads other schemas (inventory low/out forwarded live). Reports `/reports/sales|products|customers|inventory` (ADMIN), auto-wired CMS BEST_SELLERS. **Audit finalized** — `GET /audit` queryable log (ADMIN, filters + pagination). Migration applied; build+lint+**96 unit tests** + real e2e app-boot green. Design: `docs/PHASE_11_REPORTING.md`. |
| 12 | Search & hardening | ◼ done — faceted `GET /products/search` over the dynamic attribute model behind a swappable `SEARCH_PROVIDER` port (Postgres tsvector+GIN full-text default; ES adapter scaffolded, `SEARCH_DRIVER` flip = zero caller change), conjunctive facets (PAV∪VAV attrs + brand/category/price), **keyset/cursor pagination**. Redis **cache-aside `CacheService.getOrSet` with single-flight stampede protection** (NX-PX lock + Lua release, jittered TTL, fail-open) on homepage/product-detail/category-tree + targeted eviction on catalog/CMS writes. Migration applied (search_vector + 3 indexes); build+lint+**107 unit tests** + real e2e app-boot green; **live search smoke** (4 sorts, FTS match, keyset integrity, facets) + **live cache smoke** (20 concurrent→1 loader, del/delByPrefix) passed. Design: `docs/PHASE_12_SEARCH.md`. |
